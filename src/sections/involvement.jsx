  ...
  return (
    <button
      className={`mx-1 h-3 w-3 rounded-full transition-all duration-300 ${
        active 
          ? (isDark ? 'bg-sky-200 w-6' : 'bg-orange-900/30 w-6') 
  ...
  return (
    <button
      className={`mx-1 h-2 w-2 mb-1.5 rounded-full transition-all duration-300 ${
        active 
          ? (isDark ? 'bg-white w-4' : 'bg-orange-100 w-4') 
    ...
    <div className="mt-32 container scroll-mt-32" id="involvements">
      ...
      <div className="w-full m-auto relative group">
        <Carousel
          ...
          customLeftArrow={
            <button 
              className={`absolute hover:bg-blue-900/60 left--4 z-10 hidden md:block p-3 rounded-full hover:bg-opacity-100 transition-all duration-300 opacity-0 group-hover:opacity-100 ${
                isDark 
                  ? 'bg-gray-900 bg-opacity-70 text-white' 
                  : 'bg-gray-200/30 bg-opacity-70 text-gray-800'
              }`}
          ...
          customRightArrow={
            <button 
              className={`absolute hover:bg-blue-900/60 right-0 z-10 hidden md:block p-3 rounded-full hover:bg-opacity-100 transition-all duration-300 opacity-0 group-hover:opacity-100 ${
                isDark 
                  ? 'bg-gray-900 bg-opacity-70 text-white' 
                  : 'bg-gray-200/30 bg-opacity-70 text-gray-800'
              }`}
        ...
        >
          {involvements.map((involvement, index) => (
            <div 
              key={index} 
              className={`h-[525px] rounded-2xl transition-all duration-500 transform hover:scale-103 ${
            ...
            >
              <div className="rounded-t-2xl h-90 overflow-hidden relative">
                <Carousel
                  ...
                  customLeftArrow={
                    <button className={`absolute left-2 z-50 pointer-events-auto p-2 cursor-pointer rounded-full transition-all duration-300 ${
                      'bg-gray-400/30 hover:bg-gray-400 text-white' 
                    }`}>
                  ...
                  customRightArrow={
                    <button className={`absolute right-2 z-50 pointer-events-auto p-2 cursor-pointer rounded-full transition-all duration-300 ${
                        'bg-gray-400/30 hover:bg-gray-400 text-white' 
                    }`}>
                      <svg xmlns="http://www.w3.org/2000/svg" className="h-5 w-5" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                        <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M9 5l7 7-7 7" />
                ...
                >
                  {involvement.images.map((imgObj, imgIndex) => (
                    ...
                    >
                      ...
                      <div className={`absolute pointer-events-none bottom-0 left-0 right-0 pt-3 pb-4 transition-opacity duration-300 ${
              ...
              <div className="p-6 flex flex-col gap-4 items-center flex-1 h-[calc(550px-15rem)]">
                <h3 className={`text-xl font-extrabold text-center ${isDark ? 'text-white' : 'text-gray-800'}`}>
                  {involvement.title}
                </h3>
